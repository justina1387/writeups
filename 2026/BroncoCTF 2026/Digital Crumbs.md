e0sec | overtsleeping | BroncoCTF 2026 | OSINT

![alt text](<docs/Digital Crumbs/Screenshot 2026-07-12 at 02-07-03 BroncoCTF.png>)

> Personally solved: 3
> Attempted: 4

# Challenge Information
**`Digital Crumbs | tiffany_ttn | Easy`**

> My friends just sent me this picture of a coffee place they were at and told me to meet them at the pizza store across the street. What is the building number of that pizza shop?
> > Hint: Flag should be in the format bronco{XXXX}, where XXXX is the building number.
![alt text|280](<docs/Digital Crumbs/CoffeeMill.jpg>)

# Steps
1. Observe clear signage: Oakland's Oldest Coffee House
2. Search it, and we find a reference to the actual store name.
![alt text](<docs/Digital Crumbs/image.png>)
3. Search on Google Maps. There is a Domino's Pizza directly across the street from it.
![alt text](<docs/Digital Crumbs/image-1.png>)
<details>
<summary> Flag </summary>
bronco{3360}
</details>